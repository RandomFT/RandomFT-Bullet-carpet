...
import carpet.dispensers.CarpetDispenserBehaviors;
...
public class CarpetSettings
{
    ...
    @Rule(desc = "Convierte cobblestone en arena cuando un yunque cae encima", category = {RANDOMFT, SURVIVAL})
    public static boolean anvilTurnsCobbleToSand = false;

    @Rule(desc = "Mobs de la mansion renovables [SOLO FUNCIONA EN NUEVAS MANSIONES]", category = {RANDOMFT, SURVIVAL})
    public static boolean mansionMobsRespawn = false;

    @Rule(desc = "La netherwart crece más rápido con blaze powder", category = {RANDOMFT, SURVIVAL})
    public static boolean blazeMeal = false;

    @Rule(desc = "End Portal Frame dropea el item con Silk Touch",category = {RANDOMFT, EXPERIMENTAL})
    public static boolean silkTouchEndPortalFrame = false;

    @Rule(desc = "Permite renombrar objetos usando &x en un yunque",category = {FEATURE, RANDOMFT}
    )
    public static boolean anvilColoredNames = false;
...
//    }
    ...
    public static enum RuleCategory {
        TNT, FIX, SURVIVAL, CREATIVE, EXPERIMENTAL, OPTIMIZATIONS, FEATURE, COMMANDS, BULLET, RANDOMFT
    }
